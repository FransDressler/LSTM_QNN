...
class StockXLSTMTS(nn.Module):
    def __init__(self, input_dim=18, hidden_dim=64, latent_dim=16, output_dim=7, dropout=0.2, num_layers=2):
        super(StockXLSTMTS, self).__init__()

        self.relative_dim = 10  # Returns, volume, volatility, VIX, fear/greed, price_history, etc.
        self.company_dim = 8    # Market cap, beta, etc.
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers

        # CNN for relative features preprocessing (now includes price history)
        self.conv1 = nn.Conv1d(self.relative_dim, 28, kernel_size=3, padding=1)  # More channels for price history
        self.relu = nn.ReLU()
        ...
        combined_dim = 28 + 20  # 48 total after CNN + embedding
        self.feature_projection = nn.Linear(48, hidden_dim)
    ...
    def forward(self, x):
        ...
        # Split features
        relative_features = x[:, :, :self.relative_dim]  # (batch, 60, 10)
        company_features = x[:, :, self.relative_dim:]   # (batch, 60, 8)

        # Process relative features with CNN
        rel_conv = relative_features.permute(0, 2, 1)    # (batch, 10, 60)
        rel_conv = self.relu(self.conv1(rel_conv))       # (batch, 28, 60)
        rel_conv = self.bn1(rel_conv)
        rel_conv = self.pool(rel_conv)                   # (batch, 28, 30)
        rel_conv = rel_conv.permute(0, 2, 1)             # (batch, 30, 28)
        ...
        # Combine features
        combined = torch.cat([rel_conv, company_emb], dim=2)  # (batch, 30, 48)
        ...
        output = self.output_layer(self.dropout(latent))  # (batch, 7)

        return output

def main():
    print("🧠 STOCK X-LSTM-TS TRAINING (7-DAY PREDICTION + DENOISED PRICE HISTORY)")
    print("=" * 75)
    ...
    try:
        ...
        expected_features = 18  # 10 relative (incl. price history) + 8 asset
        if X_train.shape[-1] != expected_features:
            print(f"❌ Feature dimension mismatch! Expected {expected_features}, got {X_train.shape[-1]}")
            print("   Run preprocessing.py again to generate data with sentiment + price history features")
            return
        else:
            print(f"✅ Feature validation passed: {expected_features} features (10 relative + 8 asset)")
    ...
    # Model with X-LSTM-TS architecture for 7-day prediction + price history
    model = StockXLSTMTS(
        input_dim=18,       # 10 relative + 8 asset features
        hidden_dim=128,     # Increased for price history + 7-day complexity
        latent_dim=32,      # Larger bottleneck for 7-day output
