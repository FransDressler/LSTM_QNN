...
def calculate_relative_features(data, vix_data=None, use_denoising=True):
    """Calculate relative features including sentiment indicators and denoised price history"""
    ...
    if use_denoising:
        ...
        # Add denoised price history as normalized features
        # Normalize prices relative to the most recent price
        current_price = close_denoised.iloc[-1] if hasattr(close_denoised, 'iloc') else close_denoised[-1]
        price_history_norm = (close_denoised / current_price - 1).fillna(0)  # Relative to current price

        stored_features = {
            'close_denoised': close_denoised,
            'price_history_norm': price_history_norm
        }
    else:
        ...
        # Raw price history normalization
        current_price = data['Close'].iloc[-1]
        price_history_norm = (data['Close'] / current_price - 1).fillna(0)

        stored_features = {
            'close_denoised': data['Close'],
            'price_history_norm': price_history_norm
        }
    ...
    return {
        ...
        'price_history_norm': stored_features['price_history_norm'].values,
        'close_denoised': stored_features['close_denoised']  # Keep for targets calculation
    }
...
def main():
        ...
        try:
            ...
            def create_sequences_sliding(start_idx, end_idx):
                ...
                for i in range(start_idx, end_idx - SEQUENCE_LENGTH - PREDICTION_LENGTH + 1):
                    ...
                    # Denoised price history (1 feature) - normalized relative to current price
                    price_hist_seq = relative_features['price_history_norm'][i:i+SEQUENCE_LENGTH].reshape(SEQUENCE_LENGTH, 1)

                    # Asset features (8) für jede Sequenz
                    asset_seq = np.tile(asset_vec, (SEQUENCE_LENGTH, 1))

                    # Combine (18 total: 9 relative + 1 price_history + 8 asset)
                    full_features = np.concatenate([input_features, price_hist_seq, asset_seq], axis=1)

                    # Target: future percentage changes using denoised prices
                    if 'close_denoised' in relative_features:
                        # Use denoised prices for both current and future
                        if hasattr(relative_features['close_denoised'], 'iloc'):
                            current_price = relative_features['close_denoised'].iloc[i+SEQUENCE_LENGTH-1]
                            future_prices = relative_features['close_denoised'].iloc[i+SEQUENCE_LENGTH:i+SEQUENCE_LENGTH+PREDICTION_LENGTH]
                        else:
                            current_price = relative_features['close_denoised'][i+SEQUENCE_LENGTH-1]
                            future_prices = relative_features['close_denoised'][i+SEQUENCE_LENGTH:i+SEQUENCE_LENGTH+PREDICTION_LENGTH]
                    else:
    ...
    # Scale relative features (0:10) to [-1, +1] for stable training (9 relative + 1 price history)
    X_train_relative = X_train_combined[:, :, :10].reshape(-1, 10)
    ...
    X_val_relative = X_val_combined[:, :, :10].reshape(-1, 10)
    ...
    X_train_relative_norm = X_train_relative_norm.reshape(X_train_combined.shape[0], SEQUENCE_LENGTH, 10)
    X_val_relative_norm = X_val_relative_norm.reshape(X_val_combined.shape[0], SEQUENCE_LENGTH, 10)

    X_train_final = np.concatenate([X_train_relative_norm, X_train_combined[:, :, 10:]], axis=2)
    X_val_final = np.concatenate([X_val_relative_norm, X_val_combined[:, :, 10:]], axis=2)
    ...
    print(f"   💡 Features: 10 relative (6 base + 3 sentiment + 1 price history) + 8 asset meta = 18 total")
    print(f"   🧠 Sentiment features: VIX normalized, VIX change, Fear/Greed synthetic")
    print(f"   📈 Price history: Denoised 60-day normalized price sequence")
    print(f"   📊 Targets: 7-day percentage changes (mean={y_train_tensor.mean():.3f}%, std={y_train_tensor.std():.3f}%)")
    ...
    print(f"\n🚀 Ready for X-LSTM training with sentiment + denoised price features!")
    print(f"   💡 Features scaled to [-1, +1] for stable training")
    print(f"   🧠 VIX & Fear/Greed features help combat regime noise")
    print(f"   📈 Denoised price history provides clean trend information")
    print(f"   💡 Targets are 7-day percentage changes (directly interpretable)")
